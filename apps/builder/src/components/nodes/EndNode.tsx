...
export const EndNode = memo(function EndNode({ id, data, selected }: NodeProps) {
  ...
  const handleDelete = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    if (window.confirm('종료 노드를 삭제하시겠습니까?')) {
      removeNode(id);
    }
  }, [id, removeNode]);
  ...
  return (
    <div
      className={`group relative px-6 py-4 rounded-xl bg-gradient-to-br from-red-500 to-red-600 text-white shadow-md border-2 transition-all duration-200 min-w-[160px] ${
        selected
          ? 'border-white shadow-lg shadow-red-500/30 ring-2 ring-white/30'
          : 'border-red-600 hover:border-red-400 hover:shadow-lg'
      }`}
    >
      <Handle
        type="target"
        position={Position.Top}
        className="w-3 h-3 !bg-red-700 border-2 border-white shadow-md"
      />
